function [TreeRank, E, bels, msg_neg] = sumProdBelProg( filename )
...
function [bel_nodes, msg_neg] = BottomUpBelProp(nodes, E, TR, np, ep)
    ...
    for i = 1:num
        ...
        if sum(children)~= 0,
            ...
            msg_node = [sum(bel_children(children, :).* ep_st(:, logical([1 0 1 0])) ,2),...
                        sum(bel_children(children, :).* ep_st(:, logical([0 1 0 1])) ,2)];
            %msg_node,pause,
...
function [bel_nodes] = TopDownBelProp(nodes, bel, msg_neg, E, TR, ep)
    ...
    for i = 1:num
        ...
        if sum(children)~=0
            ...
            msg_node = [sum(bel_msg.* ep_ts(:, logical([1 1 0 0])) ,2),...
                        sum(bel_msg.* ep_ts(:, logical([0 0 1 1])) ,2)];
